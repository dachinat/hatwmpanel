// Package volume controls the default PipeWire/PulseAudio output.
package volume
...
func (c *Controller) SetPercent(percent int) {
	...
	select {
	case c.requests <- clampPercent(percent):
	default:
		select {
		case <-c.requests:
		default:
		}
		c.requests <- clampPercent(percent)
	}
}
...
func Read() (State, error) {
	...
	if output, err := exec.CommandContext(
		ctx, "wpctl", "get-volume", "@DEFAULT_AUDIO_SINK@").Output(); err == nil {
		return parseWPCTL(string(output))
	}
	output, err := exec.CommandContext(
		ctx, "pactl", "get-sink-volume", "@DEFAULT_SINK@").Output()
	if err != nil {
		return State{}, err
	}
	...
	muteOutput, _ := exec.CommandContext(
		ctx, "pactl", "get-sink-mute", "@DEFAULT_SINK@").Output()
...
func Set(percent int) error {
	...
	if err := exec.CommandContext(
		ctx, "wpctl", "set-volume", "@DEFAULT_AUDIO_SINK@", value).Run(); err == nil {
		_ = exec.CommandContext(
			ctx, "wpctl", "set-mute", "@DEFAULT_AUDIO_SINK@", "0").Run()
		return nil
	}
	if err := exec.CommandContext(
		ctx, "pactl", "set-sink-volume", "@DEFAULT_SINK@", value).Run(); err != nil {
		return err
	}
	_ = exec.CommandContext(
		ctx, "pactl", "set-sink-mute", "@DEFAULT_SINK@", "0").Run()
	return nil
}
